...
| AUD-005 | P3 | Code hygiene | NR-Ai-Backend | local | engineering | Lint reports a large warning backlog. | Run `npm run lint`. | Warning backlog is triaged and reduced over time without blocking release gates. | 512 warnings remain after the release-readiness cleanup pass; `ClientPortfolio.tsx` now has 0 warnings. | TBD | Wave 3 | Open |
