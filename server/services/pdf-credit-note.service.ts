  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (creditNote.status) {
        ...
        doc.font('Helvetica').text(creditNote.status.toUpperCase(), margin + contentWidth - 125, y + 30);
      }
      ...
      lines.forEach((line, index) => {
        ...
        doc.text(formatAmount(line.unitPrice, creditNote.currency), colX.price, y + 8, { width: 60, align: 'center' });
        doc.text(`${vatPercent}%`, colX.vat, y + 8, { width: 40, align: 'center' });
        doc.text(formatAmount(lineTotal, creditNote.currency), colX.amount - 60, y + 8, { width: 60, align: 'right' });
      ...
      doc.text(formatAmount(creditNote.subtotal, creditNote.currency), totalsValueX - 80, y, { width: 80, align: 'right' });
      y += 18;

      doc.text('VAT:', totalsX, y);
      doc.text(formatAmount(creditNote.vatAmount, creditNote.currency), totalsValueX - 80, y, { width: 80, align: 'right' });
      y += 22;
      ...
      doc.text(formatAmount(creditNote.total, creditNote.currency), totalsValueX - 80, y, { width: 80, align: 'right' });
      ...
      if (isVATRegistered) {
        doc.fontSize(7);
        doc.text(
          'This is a tax credit note - Please keep for your records',
          margin,
          footerY + 12,
