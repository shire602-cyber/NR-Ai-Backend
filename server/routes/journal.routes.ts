...
export function registerJournalRoutes(app: Express) {
  ...
  app.post("/api/companies/:companyId/journal", authMiddleware, requireCustomer, asyncHandler(async (req: Request, res: Response) => {
    ...
    // Create entry + lines atomically. If any line insert fails the whole
    // operation rolls back so we never leave an entry without balancing lines.
  ...
  app.post("/api/journal/:id/reverse", authMiddleware, requireCustomer, asyncHandler(async (req: Request, res: Response) => {
    ...
    // Create reversing entry with swapped debits/credits — atomic so a partial
    // reversal (entry without all balancing lines) cannot corrupt the ledger.
    const now = new Date();
